...
const int32_t N_TESTIMAGES = 3411;
...
// Outer:Easy, Medium, Hard
  // inner: car, ped, cyclist
...
bool eval(string kitti_dir, string result_dir, Mail* mail){
  // set some global parameters
  initGlobals();

  // ground truth and result directories
  string gt_dir         = kitti_dir+"/label_2/";
  // string result_dir     = kitti_dir+"/"+run_name+"/";
  ...
  std::vector<int> line;
  std::ifstream file("ImageSets/val.txt");

  std::string mystr;

  while(file >> mystr)
  {
      line.push_back(std::atoi(mystr.c_str()));
  }
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // if (argc!=2 && argc!=4) {
  //   cout << "Usage: ./eval_detection kitti_dir run_name" << endl;
  //   return 1;
  // }
  // read arguments
  string kitti_dir = argv[1];
  ...
  // run evaluation
  if (eval(kitti_dir,run_name, mail)) {
    mail->msg("Your evaluation results are available at:");
    mail->msg("http://www.cvlibs.net/datasets/kitti/user_submit_check_login.php?benchmark=object&user=%s&result=%s",argv[3], kitti_dir.c_str());
  } else {
    system(("rm -r results/" + kitti_dir).c_str());
